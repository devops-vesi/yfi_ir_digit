sap.ui.define([
	...
	return BaseObject.extend("vesi.fi.rfi.yfi_dfi_rfi.model.GroupSortState", {
		 ...
		 * @alias vesi.fi.rfi.yfi_dfi_rfi.model.GroupSortState
		 */
		...
		/**
		 * Sorts by ErnamNom, or by Zzwrbtr
		 *
		...
		/**
		 * Groups by Zzwrbtr, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Zzwrbtr") {
				...
				this._oViewModel.setProperty("/sortBy", "Zzwrbtr");

				aSorters.push(
					new Sorter("Zzwrbtr", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "ErnamNom");
			}
